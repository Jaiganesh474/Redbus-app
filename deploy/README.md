...
This guide explains how to deploy your **Frontend (Next.js)**, **Backend (Spring Boot)**, and **Database (MySQL)** onto an **AWS EC2 Free Tier (`t2.micro` or `t3.micro`)** instance with automatic GitHub Actions CI/CD.
...
3. **Security Group Inbound Rules** (open ports):
...
```bash
ssh -i "redbus-key.pem" ubuntu@<YOUR_EC2_PUBLIC_IP>
```
...
| `EC2_USERNAME` | `ubuntu` |
| `EC2_SSH_KEY` | Entire content of your `redbus-key.pem` private key file |
