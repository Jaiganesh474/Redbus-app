...
export default function HeroSearch() {
  ...
  const handleNlpSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const busTypeParam = res.busType ? `&busType=${encodeURIComponent(res.busType)}` : "";
      const maxPriceParam = res.maxPrice ? `&maxPrice=${res.maxPrice}` : "";

      router.push(`/search?source=${encodeURIComponent(src)}&destination=${encodeURIComponent(dst)}&date=${encodeURIComponent(dt)}${busTypeParam}${maxPriceParam}`);
    } catch (error) {
      console.error("NLP query failed, falling back to manual search", error);
      handleSearch();
    } finally {
  ...
  return (
    <div className="relative">
      {/* Background Graphic */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#d84e55] via-[#c63f46] to-[#a82d33] rounded-3xl shadow-xl overflow-hidden -z-10">
      ...
      <div className="px-4 py-6 sm:px-6 sm:py-12 md:py-16 text-white max-w-5xl mx-auto">
        {/* Title */}
        <div className="text-center max-w-2xl mx-auto mb-5 sm:mb-8">
          <div className="inline-flex items-center space-x-1.5 sm:space-x-2 px-2.5 sm:px-3 py-0.5 sm:py-1 bg-white/10 backdrop-blur-md rounded-full text-[10px] sm:text-xs font-semibold tracking-wide uppercase mb-2 sm:mb-3 border border-white/15">
            ...
            <span>India&apos;s First AI-Powered Bus Booking</span>
          </div>
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tight text-white leading-tight">
            Book Bus Tickets with <span className="text-amber-300">Confidence</span>
          </h1>
          <p className="text-red-100 text-xs sm:text-base mt-1.5 sm:mt-2 font-normal">
          ...
          <form onSubmit={handleNlpSearch} className="relative">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-9 sm:pl-12 pr-24 sm:pr-36 py-2.5 sm:py-3.5 bg-white dark:bg-slate-900 text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-slate-400 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-medium shadow-md sm:shadow-lg border border-red-100 dark:border-slate-800 focus:outline-hidden focus:ring-4 focus:ring-amber-300/50"
              />
              <button
                type="submit"
                disabled={isNlpLoading}
                className="absolute right-1 sm:right-2 px-2.5 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-[#d84e55] to-red-600 hover:from-[#b83e44] hover:to-red-700 text-white rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold transition-all shadow-md flex items-center space-x-1 disabled:opacity-70 cursor-pointer"
              >
                {isNlpLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-3.5 sm:p-5 shadow-xl sm:shadow-2xl text-gray-800 dark:text-slate-100 border border-gray-100 dark:border-slate-800 max-w-4xl mx-auto">
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-12 gap-2.5 sm:gap-3 items-center">
            ...
            <div className="flex md:hidden justify-center -my-1 relative z-10">
              <button
                ...
                onClick={handleSwap}
                title="Swap Cities"
                className="px-3 py-1 bg-gray-50 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-slate-700 text-[#d84e55] dark:text-red-400 border border-gray-200 dark:border-slate-700 rounded-full text-[11px] font-bold shadow-2xs flex items-center space-x-1 transition-all active:scale-95 cursor-pointer"
              >
            ...
            <div className="md:col-span-3 relative group">
              <div className="flex items-center justify-between mb-1">
                <label className="block text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider flex items-center space-x-1">
                  <Calendar className="w-3.5 h-3.5 text-gray-600 dark:text-slate-400" />
                  <span>Date</span>
                </label>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 sm:py-2.5 bg-gray-50 dark:bg-slate-800 hover:bg-gray-100/80 dark:hover:bg-slate-700/80 focus:bg-white dark:focus:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 text-xs sm:text-sm font-semibold text-gray-900 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-[#d84e55] focus:border-transparent transition-all"
              />
            </div>
            ...
            <div className="md:col-span-12 mt-1 sm:mt-2">
              <button
                type="submit"
                className="w-full py-3 sm:py-3.5 bg-[#d84e55] hover:bg-[#b83e44] text-white rounded-xl font-bold text-sm sm:text-base shadow-md sm:shadow-lg shadow-red-500/25 transition-all flex items-center justify-center space-x-2 active:scale-[0.99] cursor-pointer"
              >
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mt-4 sm:mt-6 text-[11px] sm:text-xs text-white/90">
          ...
          {[
            ...
            <button
              ...
              className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full transition-colors flex items-center space-x-1"
            >
              <span>{pair.from}</span>
